...
object RegistrationPage {
  def asParent: Region = new VBox {
    ...
    val registrationCard = new GridPane {
      ...
      add(logoView, 0, 0, 2, 1)
      scalafx.scene.layout.GridPane.setHalignment(logoView, scalafx.geometry.HPos.Center)
      scalafx.scene.layout.GridPane.setMargin(logoView, Insets(0, 0, 20, 0))
      add(userLabel, 0, 1)
      add(userField, 1, 1)
      add(stateLabel, 0, 2, 2, 1)
      GridPane.setHalignment(stateLabel, scalafx.geometry.HPos.Center)
      
      add(registerBtn, 0, 3, 2, 1)
    ...
    children = Seq(backBtnContainer, registrationCard)
  }
}